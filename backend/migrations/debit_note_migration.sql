CREATE TABLE debit_notes (
    ...
    voucher_no      VARCHAR(30) UNIQUE NOT NULL,       -- format: DN-26001
    voucher_date    DATE NOT NULL,
    ref_bill_id     INTEGER REFERENCES purchase_bills(bill_id),
    ref_bill_no     VARCHAR(50),
    ref_bill_date   DATE,
    gl_party_id     INTEGER NOT NULL REFERENCES gl_master(gl_id),  -- Supplier GL
    party_name      VARCHAR(200) NOT NULL,
    gl_debit_id     INTEGER REFERENCES gl_master(gl_id),           -- Purchase Returns A/c
...
CREATE INDEX idx_dn_party ON debit_notes(gl_party_id);
CREATE INDEX idx_dn_fy    ON debit_notes(fy_code);
CREATE INDEX idx_dni_dn   ON debit_note_items(dn_id);
