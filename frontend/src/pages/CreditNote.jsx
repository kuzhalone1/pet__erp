...
export default function CreditNote() {
  ...
  // Next voucher number
  const [nextVoucherNo, setNextVoucherNo] = useState('CN-Auto');
  ...
  const fetchNextVoucherNo = async (fyCode) => {
    try {
      const res = await api.get('/accounts/credit-notes', { params: { fy_code: fyCode } });
      const count = (res.data || []).length;
      const fyShort = fyCode.replace('-', '').slice(2);
      setNextVoucherNo(`CN-${fyShort}${String(count + 1).padStart(5, '0')}`);
    } catch { setNextVoucherNo('CN-Auto'); }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const creditAcc = glAccounts.find(g => g.gl_id === form.gl_credit_id);
      ...
      const payload = {
        ...
        gl_credit_id: form.gl_credit_id || null,
        credit_desc: creditAcc?.gl_name || form.credit_desc || null,
      ...
      toast.success('Credit Note saved and GL posted!');
      resetForm();
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const res = await api.get('/accounts/credit-notes', { params });
      setHistory(res.data);
  ...
  const handleCancel = async (id) => {
    if (!window.confirm('Cancel this credit note? GL postings will be reversed.')) return;
    try {
      await api.delete(`/accounts/credit-notes/${id}`);
      toast.success('Credit Note cancelled — GL reversed');
      fetchHistory();
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <h1 className="text-2xl font-black text-slate-800 flex items-center gap-3">
          <ReceiptText className="p-2 bg-rose-50 text-rose-600 rounded-xl" size={24} />
          Credit Note
          <span className="text-sm font-normal text-slate-400 ml-1">/ Sales Return</span>
        </h1>
        <div className="flex bg-slate-100 p-1 rounded-xl w-fit shadow-inner">
          <button onClick={() => setActiveTab('new')}
            className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2
              ${activeTab === 'new' ? 'bg-white text-rose-600 shadow-sm' : 'text-slate-500'}`}>
            <Plus size={14} /> New
          </button>
          <button onClick={() => { setActiveTab('history'); }}
            className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2
              ${activeTab === 'history' ? 'bg-white text-rose-600 shadow-sm' : 'text-slate-500'}`}>
      ...
      {activeTab === 'new' && (
        <div className="space-y-5">
          ...
          <div className="p-4 bg-rose-50 border border-rose-100 rounded-2xl space-y-4">
            <p className="text-xs font-black text-rose-500 uppercase tracking-widest">Voucher Info</p>

            <div className="flex flex-wrap gap-4">
              {/* Voucher No */}
              <div className="flex-1 min-w-[150px]">
                <label className="label flex items-center gap-1 text-rose-700"><Hash size={13} /> Voucher No</label>
                <input readOnly value={nextVoucherNo} className="input-field h-10 w-full font-mono bg-white text-rose-700 font-bold cursor-not-allowed" />
              </div>
              {/* FY */}
              ...
              {/* Ref Bill No */}
              <div className="flex-1 min-w-[150px]">
                <label className="label flex items-center gap-1"><FileText size={13} /> Ref Sale Bill No</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-4 bg-white border border-slate-100 rounded-2xl shadow-sm space-y-3">
              ...
              <div>
                ...
                <SearchableSelect
                  ...
                  placeholder="Search customer GL account..."
                />
              </div>
            ...
            <div className="p-4 bg-white border border-slate-100 rounded-2xl shadow-sm space-y-3">
              <p className="text-xs font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                <Tag size={13} /> Credit Head & GST
              </p>
              <div>
                <label className="label">Credit/Sales Return Account</label>
                <SearchableSelect
                  options={glAccounts}
                  value={form.gl_credit_id}
                  onChange={(id) => {
                    const acc = glAccounts.find(g => g.gl_id === id);
                    setForm(f => ({ ...f, gl_credit_id: id, credit_desc: acc?.gl_name || '' }));
                  }}
                  placeholder="Search sales return GL..."
                />
                <p className="text-[10px] text-slate-400 mt-1">Usually a "Sales Returns" or "Sales Discount" account</p>
              </div>
              ...
              <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl">
                <label className="relative inline-flex items-center cursor-pointer">
                  ...
                  <div className="w-10 h-5 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:bg-rose-500 after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-5" />
                </label>
                <div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <div className="px-4 py-3 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
              ...
              <button onClick={addItem}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-rose-600 text-white text-xs font-bold rounded-lg hover:bg-rose-700 transition-all">
                    ...
                    return (
                      <tr key={item._id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-2 py-1.5">
                          <input type="text" className="w-full px-2 py-1.5 border border-slate-200 rounded-lg text-xs font-mono focus:outline-none focus:ring-2 focus:ring-rose-400"
                            value={item.hsn_code}
                            onChange={e => updateItem(item._id, { hsn_code: e.target.value })}
                            placeholder="HSN" />
                        </td>
                        <td className="px-2 py-1.5">
                          <input type="text" list="units-list" className="w-full px-2 py-1.5 border border-slate-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-rose-400"
                            value={item.unit}
                        ...
                        <td className="px-2 py-1.5">
                          <input type="number" min="0" step="0.001"
                            className="w-full px-2 py-1.5 border border-slate-200 rounded-lg text-sm text-right focus:outline-none focus:ring-2 focus:ring-rose-400"
                            value={item.quantity}
                        ...
                        <td className="px-2 py-1.5">
                          <select className="w-full px-1 py-1.5 border border-slate-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-rose-400"
                            value={item.gst_pct}
                        ...
                        <td className="px-2 py-1.5 text-right font-bold text-rose-700">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100">
            ...
            <div className="p-3 bg-white rounded-xl border border-slate-100 shadow-sm">
              ...
              <p className="text-lg font-black text-rose-600">-₹{fmt(computed.totalDisc + computed.billDiscAmt)}</p>
            </div>
            <div className="p-3 bg-white rounded-xl border border-slate-100 shadow-sm">
            ...
            <div className="p-3 bg-rose-600 rounded-xl shadow-sm col-span-2 md:col-span-1">
              <p className="text-[10px] text-rose-200 uppercase font-bold mb-0.5">Net Amount</p>
          ...
          <div className="flex gap-4">
            ...
            <button
              ...
              className="flex-[2] flex items-center justify-center gap-2 px-8 py-3 bg-rose-600 text-white font-black uppercase text-xs tracking-widest rounded-2xl hover:bg-rose-700 disabled:bg-slate-300 disabled:cursor-not-allowed transition-all">
              {saving ? 'Saving...' : <><Save size={16} /> Save Credit Note</>}
            </button>
          </div>
          ...
          {!form.gl_party_id && (
            <div className="flex items-start gap-3 p-3 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-700">
              ...
              <span><strong>Party GL Account required.</strong> Select a customer GL account above before saving.</span>
            </div>
          )}
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-4 p-4 bg-slate-50 rounded-xl border border-dashed">
            ...
            <button onClick={fetchHistory}
              className="flex items-center gap-2 px-4 py-2 bg-rose-600 text-white rounded-lg text-sm font-bold hover:bg-rose-700 transition-all">
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-100">
                {loadingHistory
                  ...
                  : history.map(rec => {
                    const isExpanded = expandedRow === rec.cn_id;
                    const gstTotal = (rec.cgst_amount || 0) + (rec.sgst_amount || 0) + (rec.igst_amount || 0);
                    return (
                      <React.Fragment key={rec.cn_id}>
                        <tr className="hover:bg-slate-50 transition-colors">
                          <td className="px-4 py-3 font-mono text-rose-600 text-sm font-bold">{rec.voucher_no}</td>
                          ...
                          <td className="px-4 py-3 text-right font-black text-rose-700">₹{fmt(rec.net_amount)}</td>
                          <td className="px-4 py-3">{statusBadge(rec.status)}</td>
                          <td className="px-4 py-3 text-center">
                            <div className="flex items-center justify-center gap-1">
                              <button onClick={() => setExpandedRow(isExpanded ? null : rec.cn_id)}
                                title={isExpanded ? 'Hide items' : 'View items'}
                                className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors">
                                {isExpanded ? <EyeOff size={15} /> : <Eye size={15} />}
                              </button>
                              {rec.status === 'Confirmed' && (
                                <button onClick={() => handleCancel(rec.cn_id)} title="Cancel"
                                  className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors">
                                  <Trash2 size={15} />
                                </button>
                              )}
                        ...
                        {isExpanded && (
                          <tr>
                            <td colSpan="10" className="px-4 pb-4 bg-rose-50">
                              <div className="rounded-xl border border-rose-100 overflow-hidden mt-1">
                                <div className="px-4 py-2 bg-rose-100 border-b border-rose-200">
                                  <p className="text-xs font-black text-rose-600 uppercase tracking-widest">
                                    Items — {rec.voucher_no}
                                    {rec.narration && <span className="ml-3 font-normal normal-case text-rose-400">{rec.narration}</span>}
                                  </p>
                                </div>
                                  ...
                                  <table className="w-full text-sm bg-white">
                                      ...
                                      {rec.items.map((it, li) => {
                                        ...
                                        return (
                                          <tr key={li} className="hover:bg-slate-50">
                                            ...
                                            <td className="px-3 py-1.5 text-right font-bold text-rose-700">₹{fmt(it.line_total)}</td>
                                          </tr>
                                        );
                                      })}
                                    </tbody>
                                    <tfoot>
                                      <tr className="border-t-2 border-rose-200 bg-rose-50">
                                        ...
                                        <td className="px-3 py-2 text-right font-black text-rose-700">₹{fmt(rec.net_amount)}</td>
                                      </tr>
                                    </tfoot>
                                  </table>
                                ) : (
