...
export default function BankArrivals() {
  ...
  const fetchNextVoucherNo = async (fyCode) => {
    ...
    try {
      const res = await api.get('/accounts/bank-arrivals/next-voucher-no', {
        params: { fy_code: fyCode }
      });
      setNextVoucherNo(res.data.voucher_no);
    } catch {
      setNextVoucherNo('BA-Auto');
    } finally {
  ...
  useEffect(() => {
    api.get('/ledger/gl')
       .then(res => {
         const allGl = res.data;
         // Load all GL accounts for both dropdowns — let the user choose freely
         setParties(allGl);
         setCashbanks(allGl);
       })
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const res = await api.get('/accounts/bank-arrivals', { params });
      setHistory(res.data);
    } catch {
      toast.error('Error loading bank arrivals');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    if (!form.gl_bank_id) return toast.error('Bank selection required');
    ...
    const cb = cashbanks.find(c => String(c.gl_id) === String(form.gl_bank_id));
    ...
    try {
      const payload = {
        ...
        gl_bank_id: form.gl_bank_id,
        bank_name: cb?.gl_name || '',
        amount: Number(form.amount),
        ref_doc_no: form.ref_doc_no || null,
        ref_doc_date: form.ref_doc_date || null,
        narration: form.narration
      };

      if (editingId) {
        await api.put(`/accounts/bank-arrivals/${editingId}`, {
          voucher_date: form.voucher_date,
          ref_doc_no: form.ref_doc_no || null,
          ref_doc_date: form.ref_doc_date || null,
          narration: form.narration
        });
        toast.success('Bank Arrival updated');
      } else {
        await api.post('/accounts/bank-arrivals/', payload);
        toast.success('Bank Arrival created');
      }
      resetForm();
      setActiveTab('history');
    } catch (err) {
      toast.error(err.response?.data?.detail || 'Error saving bank arrival');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleEdit = (record) => {
    setEditingId(record.arrival_id);
    ...
    setForm({
      ...
      gl_bank_id: record.gl_bank_id || '',
      amount: record.amount,
      ref_doc_no: record.ref_doc_no || '',
      ref_doc_date: record.ref_doc_date?.split('T')[0] || '',
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Cancel this bank arrival?')) return;
    try {
      await api.delete(`/accounts/bank-arrivals/${id}`);
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <h1 className="text-2xl font-black text-slate-800 flex items-center gap-2">
          <Banknote className="p-2 bg-indigo-50 text-indigo-600 rounded-xl" size={24} />
          Bank Arrivals
        </h1>
      ...
      {activeTab === 'new' ? (
        <div className="grid grid-cols-1 gap-6">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="label flex items-center gap-1"><Banknote size={14} /> Bank Account *</label>
              <SearchableSelect
                placeholder="Select or type Bank account..."
                options={cashbanks.map(c => ({ id: c.gl_id, name: c.gl_name, code: c.gl_code }))}
                value={form.gl_bank_id}
                onChange={val => setForm({ ...form, gl_bank_id: val })}
              />
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4 p-4 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
            <div>
              <label className="label flex items-center gap-1">
                ...
                <span className="text-xs text-slate-400 font-normal ml-1">(Cheque / Advice / UTR No)</span>
              </label>
              <input
                type="text"
                className="input-field h-10 w-full font-mono"
                value={form.ref_doc_no}
                onChange={e => setForm({ ...form, ref_doc_no: e.target.value })}
                placeholder="e.g. CHQ-001234"
              />
            </div>
            <div>
              <label className="label flex items-center gap-1">
                <Calendar size={14} /> Ref Date
              </label>
              <input
                type="date"
                className="input-field h-10 w-full"
                value={form.ref_doc_date}
                onChange={e => setForm({ ...form, ref_doc_date: e.target.value })}
              />
            </div>
          </div>
          ...
          <div className="flex gap-4">
            ...
            >
              {saving ? 'Processing...' : <><Save size={16} className="inline mr-1" /> {editingId ? 'Update' : 'Create'} Bank Arrival</>}
            </button>
          </div>
        </div>
      ) : (
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  ...
                  history.map(rec => (
                    <tr key={rec.arrival_id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-4 py-3 font-mono text-indigo-600 text-sm">{rec.voucher_no || '—'}</td>
                      <td className="px-4 py-3">
                        <div className="font-semibold text-slate-700">{rec.party_name || rec.gl_party_id}</div>
                      </td>
                      <td className="px-4 py-3 text-sm text-slate-600">
                        {rec.voucher_date ? new Date(rec.voucher_date).toLocaleDateString('en-IN') : '—'}
                      </td>
                      <td className="px-4 py-3 text-sm font-mono text-slate-600">{rec.ref_doc_no || '—'}</td>
                      <td className="px-4 py-3 text-sm text-slate-600">
                        {rec.ref_doc_date ? new Date(rec.ref_doc_date).toLocaleDateString('en-IN') : '—'}
                      </td>
                      <td className="px-4 py-3 text-right font-bold text-slate-700">₹{Number(rec.amount).toLocaleString('en-IN')}</td>
                      <td className="px-4 py-3 text-right text-slate-500">₹{Number(rec.balance).toLocaleString('en-IN')}</td>
                      <td className="px-4 py-3">
                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider
                          ${rec.status === 'Completed' ? 'bg-emerald-100 text-emerald-700' :
                            rec.status === 'Cancelled' ? 'bg-rose-100 text-rose-700' :
                            rec.status === 'Matched' ? 'bg-blue-100 text-blue-700' :
                      ...
                      <td className="px-4 py-3 text-right flex gap-2 justify-end">
                        {rec.status === 'Open' && (
                          <>
                            ...
                            <button
                              title="Cancel"
                              onClick={() => handleDelete(rec.arrival_id)}
